...
docker-build: ## Build a container image and add the version and latest tag
	@if command -v docker > /dev/null; then \
		echo "Building..."; \
		docker buildx build -t jsfelipearaujo/user-service:latest -t jsfelipearaujo/user-service:$$(git rev-parse --short HEAD) .; \
	else \
...
docker-push: ## Push the container image to the registry
	echo "Pushing..."; \
	docker push jsfelipearaujo/user-service:latest && \
	docker push jsfelipearaujo/user-service:$$(git rev-parse --short HEAD)
